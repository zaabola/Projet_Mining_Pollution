...
function setupImageDetection() {
    ...
    btn.addEventListener('click', async () => {
        ...
        try {
            ...
            const mode = document.getElementById('page-mode') ? document.getElementById('page-mode').value : 'ppe';
            let targetModel = 'helmet';
            if (mode === 'fish') targetModel = 'fish';
            else if (mode === 'animaux') targetModel = 'animaux';
            else if (mode === 'illegal_mining') targetModel = 'ahmed';
            else if (mode === 'smoke') targetModel = 'smoke';
            ...
            if (d.error) { showError('image-error-alert', d.error); } else {
                ...
                if (mode === 'illegal_mining') {
                    if (d.detections && d.detections.length) {
                        h += '<div class="alert" style="color:red; font-size: 1.2em; font-weight: bold; text-align: center;">⚠ WARNING: Illegal mining detected in this area!</div>';
                    } else {
                        h += '<div class="alert" style="color:green; font-size: 1.2em; font-weight: bold; text-align: center;">✅ No illegal mining here. Area is safe.</div>';
                    }
                } else {
                    if (d.detections && d.detections.length) { 
                        h += `<strong>Detections: ${d.detections.length}</strong><br>`; 
                        d.detections.forEach(det => { h += `<div class="detection-result"><span class="class-name">${det.class_name}</span></div>`; }); 
                    } else { 
                        h += '<div class="alert" style="color:#666;">No objects detected</div>'; 
                    }
                }
